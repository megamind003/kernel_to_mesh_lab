...
int crc32_verify(const uint8_t *data, size_t len, uint32_t expected) {
  uint32_t calculated = crc32_calculate(data, len);
  return (calculated == expected) ? 0 : -1;
}

int recovery_encode_block(const uint8_t *input, size_t len,
                          ChecksummedBlock **output) {
  ChecksummedBlock *block = malloc(sizeof(ChecksummedBlock) + len);
  if (!block)
    return -1;

  block->crc = crc32_calculate(input, len);
  memcpy(block->data, input, len);
...
int recovery_decode_block(const ChecksummedBlock *input, uint8_t *output,
                          size_t *len) {
  *len = sizeof(ChecksummedBlock);

  if (crc32_verify(input->data, *len, input->crc) != 0) {
    return -1;
  }

  memcpy(output, input->data, *len);
  return 0;
}
